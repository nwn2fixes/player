...
void ActionPsionicMB(object oSpellTarget)
{
    ...
    object oTarget;
    effect eCone;
    //effect eVis = EffectVisualEffect(VFX_IMP_SONIC);	// old NWN1 VFX
    effect eVis = EffectVisualEffect( VFX_CREATURE_ABILITY_DUR_MINDFLAYER );	// new NWN2 VFX
    ...
    while(GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
            ...
            if (WillSave(oTarget, nDC) < 1)
            {
                ...
                DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eVis, OBJECT_SELF, 2.0));
...
void ActionPsionicCharm(object oSpellTarget)
{
    ...
    //effect eVis = EffectVisualEffect(VFX_IMP_CONFUSION_S);	// old NWN1 VFX
    effect eVis = EffectVisualEffect( VFX_DUR_SPELL_CONFUSION );	// new NWN2 VFX
