...
//:://////////////////////////////////////////////
// ChazM 5/31/07 Get2DAStringOrDefault() now in ginc_2da
// kevL 161009 - fix prototype/definition mismatch for WMSetAreaTable().
...
// variable names
const string VAR_WM_SPAWNPOINT_MAX_DIST		= "WM_SPAWNPOINT_MAX_DIST";

// default values 
const float WM_DEFAULT_SPAWNPOINT_MAX_DIST = 25.0f;
...
//                  (default = 0, use value in 2da) // kL_fix: 'x2_mod_def_rest' relies on a local_int which if left unset will be 0.
//
// *  NOTE: You can call WMSetAreaProbability later to change the probability
//    of having an encounter
void WMSetAreaTable(object oArea, string sTableName, int bUseDoors = FALSE, int nListenCheckDC = 0); // kL_fix
...
location WMGetAmbushSpot(object oPC)
{
	...
	if (bUseDoors)
	{
        ...
        //----------------------------------------------------------------------
        object oDoor = GetFirstObjectInShape(SHAPE_SPHERE, fMaxDist, GetLocation(oPC), TRUE, OBJECT_TYPE_DOOR );
        object oDoor2 = GetNextObjectInShape(SHAPE_SPHERE, fMaxDist,GetLocation(oPC),TRUE,OBJECT_TYPE_DOOR);
